...
try {
    $stmt = $pdo->prepare("
        SELECT id, country, section_header, channel, ch_name, ch_value, withdraw_currency, channel_options,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($section === 'dashboard') {
            ...
            if (empty($country) || empty($section_header) || empty($channel) || empty($ch_name) || empty($ch_value) || empty($withdraw_currency) || empty($channel_options)) {
                ...
                if ($stmt->fetch()) {
                    ...
                    $stmt = $pdo->prepare("UPDATE region_settings SET country = ?, section_header = ?, channel = ?, ch_name = ?, ch_value = ?, withdraw_currency = ?, channel_options = ? WHERE id = ?");
                    $stmt->execute([$country, $section_header, $channel, $ch_name, $ch_value, $withdraw_currency, $channel_options, $id]);
...
<head>
    ...
    <style>
        ...
        .edit-form input[type="text"], .edit-form input[type="number"], .edit-form input[type="file"], .edit-form select, .edit-form input[type="checkbox"], .edit-form textarea { width: 100%; padding: 8px; border: 1px solid #ddd; border-radius: 4px; font-size: 13px; box-sizing: border-box; }
    ...
    <div class="dashboard-container">
        ...
        <?php if ($section === 'dashboard'): ?>
            <form action="" method="POST" class="edit-form">
                ...
                <input type="text" name="section_header" value="<?php echo htmlspecialchars($setting['section_header']); ?>" required placeholder="Section Header">
                <input type="text" name="channel" value="<?php echo htmlspecialchars($setting['channel']); ?>" required placeholder="Channel">
                <input type="text" name="ch_name" value="<?php echo htmlspecialchars($setting['ch_name']); ?>" required placeholder="Channel Name">
                <input type="text" name="ch_value" value="<?php echo htmlspecialchars($setting['ch_value']); ?>" required placeholder="Channel Value">
                <input type="text" name="withdraw_currency" value="<?php echo htmlspecialchars($setting['withdraw_currency'] ?? ''); ?>" required placeholder="Withdraw Currency">
                <input type="text" name="channel_options" value="<?php echo htmlspecialchars($setting['channel_options'] ?? ''); ?>" required placeholder="Channel Options">
        ...
        <?php elseif ($section === 'verification'): ?>
            <form action="" method="POST" class="edit-form" enctype="multipart/form-data">
                <input type="text" name="verify_ch" value="<?php echo htmlspecialchars($setting['verify_ch'] ?? ''); ?>" required placeholder="Verify Channel">
                <input type="text" name="vc_value" value="<?php echo htmlspecialchars($setting['vc_value'] ?? ''); ?>" required placeholder="VC Value">
                <input type="text" name="verify_ch_name" value="<?php echo htmlspecialchars($setting['verify_ch_name'] ?? ''); ?>" required placeholder="Verify Channel Name">
                <input type="text" name="verify_ch_value" value="<?php echo htmlspecialchars($setting['verify_ch_value'] ?? ''); ?>" required placeholder="Verify Channel Value">
                <input type="text" name="verify_medium" value="<?php echo htmlspecialchars($setting['verify_medium'] ?? ''); ?>" placeholder="Verify Medium">
                <input type="text" name="vcn_value" value="<?php echo htmlspecialchars($setting['vcn_value'] ?? ''); ?>" required placeholder="VCN Value">
                <input type="text" name="vcv_value" value="<?php echo htmlspecialchars($setting['vcv_value'] ?? ''); ?>" required placeholder="VCV Value">
                <input type="text" name="verify_currency" value="<?php echo htmlspecialchars($setting['verify_currency'] ?? ''); ?>" required placeholder="Verify Currency">
                <input type="number" name="verify_amount" value="<?php echo htmlspecialchars($setting['verify_amount'] ?? ''); ?>" step="0.01" required placeholder="Verify Amount">
                <input type="number" name="rate" value="<?php echo htmlspecialchars($setting['rate'] ?? ''); ?>" step="0.01" required placeholder="Rate">
