        ...
        if ($section === 'dashboard') {
            ...
            if (empty($country) || empty($section_header) || empty($channel) || empty($ch_name) || empty($ch_value) || empty($withdraw_currency)) {
                ...
                if ($stmt->fetch()) {
                    ...
                    $stmt = $pdo->prepare("UPDATE region_settings SET country = ?, section_header = ?, channel = ?, channel_options = ?, ch_name = ?, ch_value = ?, withdraw_currency = ? WHERE id = ?");
                    $stmt->execute([$country, $section_header, $channel, $channel_options, $ch_name, $ch_value, $withdraw_currency, $id]);
...
<head>
    ...
    <style>
        ...
        .edit-form input[type="text"], .edit-form input[type="number"], .edit-form input[type="file"], .edit-form select, .edit-form textarea, .edit-form input[type="checkbox"] { width: 100%; padding: 8px; border: 1px solid #ddd; border-radius: 4px; font-size: 13px; box-sizing: border-box; }
        .edit-form textarea { resize: vertical; min-height: 80px; font-family: inherit; }
    ...
    <div class="dashboard-container">
        ...
        <?php if ($section === 'dashboard'): ?>
            <form action="" method="POST" class="edit-form">
                ...
                <input type="text" name="section_header" value="<?php echo htmlspecialchars($setting['section_header']); ?>" placeholder="Section Header" required>
                <input type="text" name="channel" value="<?php echo htmlspecialchars($setting['channel']); ?>" placeholder="Channel Label" required>
                <textarea name="channel_options" placeholder="Channel Options (One option per line)"><?php echo htmlspecialchars($setting['channel_options'] ?? ''); ?></textarea>
                <input type="text" name="ch_name" value="<?php echo htmlspecialchars($setting['ch_name']); ?>" placeholder="Channel Name Label" required>
                <input type="text" name="ch_value" value="<?php echo htmlspecialchars($setting['ch_value']); ?>" placeholder="Channel Value Label" required>
                <input type="text" name="withdraw_currency" value="<?php echo htmlspecialchars($setting['withdraw_currency'] ?? ''); ?>" placeholder="Withdraw Currency" required>
