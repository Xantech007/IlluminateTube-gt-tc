...
// Pre-defined available payment channels/options
$channel_options = ['MTN', 'Telecel', 'AT Money'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($section === 'dashboard') {
            ...
            // Format selected channels into newline-separated string
            $selected_vc = isset($_POST['vc_value']) && is_array($_POST['vc_value']) ? $_POST['vc_value'] : [];
            $vc_value = implode("\n", array_map('trim', $selected_vc));
...
// Convert stored newline string back into array for UI population
$saved_vc_values = !empty($setting['vc_value']) ? array_map('trim', explode("\n", $setting['vc_value'])) : [];
?>
...
<head>
    ...
    <style>
        ...
        .edit-form select[multiple] { height: 90px; }
        ...
        .field-group label { font-size: 12px; color: #666; font-weight: bold; display: block; margin-bottom: 4px; }
        ...
        <?php elseif ($section === 'verification'): ?>
            <form action="" method="POST" class="edit-form" enctype="multipart/form-data">
                ...
                <div class="field-group">
                    <label>Select Channel Options (Hold Ctrl/Cmd to select multiple):</label>
                    <select name="vc_value[]" multiple required>
                        <?php foreach ($channel_options as $option): ?>
                            <option value="<?php echo htmlspecialchars($option); ?>" <?php echo in_array($option, $saved_vc_values) ? 'selected' : ''; ?>>
